...
/*   Updated: 2022/09/23 21:07:40 by acapela-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
j != len && line_splited[i][j + 1] != '$')
		{
			...
			if (threated != 0)
				ms_has_single_quotes(ms, dolar_cmd, \
ft_strdup(ms->shell_line_tokenized), threated);
			else
				ms_has_single_quotes(ms, \
dolar_cmd, ft_strdup(line_splited[i]), threated);
			threated = 1;
		}
		j++;
	}
